...
ggplot(user_mean,aes(mean)) +
  geom_histogram(binwidth = 0.05,col='white',fill="plum") + labs(x = "User Average Score")+geom_vline(xintercept = mean(user_mean$mean),col='grey',size=2)
ggplot(item_mean,aes(mean)) +
  geom_histogram(binwidth = 0.05,col='white',fill="sandybrown") + labs(x = "Item Average Score")+geom_vline(xintercept = mean(item_mean$mean),col='grey',size=2)
...
From the figures we know:
1.Rating distribution is nealy normal and sightly right skewed. Rating four has the highest frequency.
2.Distribution of user average score is normal.
3.Distribution of item average score is normal.
4.
...
#### **2. Matrix Factorization by Singular Value Decomposition**

#### 2.1 Build Recommendation Model Base On SVD Method
```{r}
...
**2.4 Making Predictions From the Decomposed Matrices**
...
#### 3. Build Recommendation Model Base On Funk SVD Method
...
**3.3 Making Predictions From the Decomposed Matrices**
